...
const Register = ({email, role}: IRegister) => {
    ...
    const { logoutSession } = useContext(AdminContext)
        ...
        <div className="w-hull h-full flex items-center justify-center">
            <form onSubmit={handleRegistration}>
                <div className="flex flex-col justify-center w-4/5 md:w-[400px] border-2 border-neutral-400 p-4 rounded-md">
                    ...
                    <Input className=" cursor-not-allowed bg-neutral-600" value={details.email} readOnly />
                    
                    <Label className="font-semibold mt-6 mb-2">Full Name:</Label>
                    <Input
                        required
                        id="name"
                        value={details.name}
                    ...
                    />
                    <Button type="submit" className="mt-4 flex items-center gap-2" variant="default">
                        {!isLoading? "Register" : <CircularProgress width={24} />}
                    </Button>
                </div>
            </form>
        </div>
